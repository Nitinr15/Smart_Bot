...
def retrieve_top_matching_content(query_text, top_k=3):
    text = clean_text(query_text)
    embeddings = embed_text(text)
    if embeddings:
        result = index.query(vector=embeddings, top_k=top_k, namespace="pdf_text", include_values=False, include_metadata=True)
        return result
    else:
